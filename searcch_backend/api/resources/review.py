...
class ReviewListAPI(Resource):
    def get(self, artifact_group_id):
        ...
        reviews = db.session.query(ArtifactReviews).filter(
            ArtifactReviews.artifact_group_id == artifact_group_id).all()
...
class ReviewAPI(Resource):
    ...
    def post(self, artifact_group_id):
        ...
        existing_review = db.session.query(ArtifactReviews).filter(ArtifactReviews.artifact_group_id == artifact_group_id, ArtifactReviews.user_id == login_session.user_id).first()

        # if it does, update the review, else add a new review
        if existing_review:
            existing_review.review = review
            existing_review.review_time = datetime.now()
            existing_review.artifact_id = artifact_group.publication.artifact_id
            message = "updated existing review"
        else:
            new_review = ArtifactReviews(
                user_id=login_session.user_id, artifact_group_id=artifact_group_id, review=review, review_time=datetime.now(), artifact_id=artifact_group.publication.artifact_id)
    ...
    def put(self, artifact_group_id):
        ...
        existing_review = db.session.query(ArtifactReviews).filter(
            ArtifactReviews.id == review_id, ArtifactReviews.user_id == login_session.user_id, ArtifactReviews.artifact_group_id == artifact_group_id).first()
