...
class StatsResource():
    ...
    def __init__(self, artifact_id, session_id):
        self.artifact_id = artifact_id
        self.session_id = session_id

    def recordView(self):

        query = db.session.query(StatsRecentViews).filter(and_(StatsRecentViews.session_id == self.session_id, StatsRecentViews.artifact_id == self.artifact_id)).first()
        ...
        if query is None:
            ...
            stats_recent_view = StatsRecentViews(
                        session_id=self.session_id, artifact_id=self.artifact_id, user_id=user_id, view_count=1
                )
